...
def test_get_war_date():
    ...
    assert warparticipation._get_war_date(pybrawl.War(created_date=raw_date_string)) == datetime.timestamp(test_date - timedelta(days=1))

    assert warparticipation._get_war_date(pybrawl.WarCurrent(state='warDay', war_end_time=raw_date_string)) == datetime.timestamp(test_date - timedelta(days=2))

    assert warparticipation._get_war_date(pybrawl.WarCurrent(state='collectionDay', collection_end_time=raw_date_string)) == datetime.timestamp(test_date - timedelta(days=1))
