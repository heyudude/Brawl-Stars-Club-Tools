...
class ScoreCalculator:
    ...
    def get_member_donations_score(self, member):
        """ Calculate the score for a given member's daily donations. 
            TODO for now set it to zero because we have no score baseline
        """

        # donation_score = member.donations_daily - \
        #     self.config['score']['min_donations_daily']

        # donation_score = donation_score if donation_score <= self.config['score'][
        #     'max_donations_bonus'] else self.config['score']['max_donations_bonus']
    ...
    def get_war_score(self, war):
        ...
        # war_score = 0
        # war_score += war.collection_battle_wins * \
        #     self.config['score']['collect_battle_won']
        # war_score += war.collection_battle_losses * \
        #     self.config['score']['collect_battle_lost']
        # war_score += war.collection_day_battles_played * \
        #     self.config['score']['collect_battle_played']
        # war_score += (3-war.collection_day_battles_played) * \
        #     self.config['score']['collect_battle_incomplete']
        ...
        # war_score += (war.battles_played - war.wins) * \
        #     self.config['score']['war_battle_lost']
